...
const Editable = ({ 
  ...
  // Safe render for when value might be undefined/null
  const safeValue = value || '';

  if (!isEditable) {
    // If not editable, render HTML safely if it contains tags, otherwise text
    // We check for HTML tags to decide whether to use dangerouslySetInnerHTML
    const hasTags = /<[a-z][\s\S]*>/i.test(safeValue);
    
    if (hasTags) {
       return <Component className={className} dangerouslySetInnerHTML={{ __html: safeValue }} />;
    }
    return <Component className={className}>{children || safeValue}</Component>;
  }
  ...
  const commonProps = {
    className: `${className} cursor-text hover:bg-yellow-200/30 hover:outline hover:outline-2 hover:outline-yellow-400/50 hover:rounded transition-all relative min-h-[1.5em]`,
    onClick: handleClick,
    title: "点击编辑富文本内容"
  };

  // Fix: Conditional rendering to avoid passing both children and dangerouslySetInnerHTML which causes React crash
  return (
    <>
      {safeValue ? (
        <Component {...commonProps} dangerouslySetInnerHTML={{ __html: safeValue }} />
      ) : (
